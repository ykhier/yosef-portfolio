            ...
            <li key={p.id} className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
              {editingId === p.id ? (
                ...
                <div className="flex items-center gap-3 p-3">
                  <img src={p.image_url} alt={p.title} className="w-16 h-12 md:w-20 md:h-14 object-cover rounded-lg flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="font-medium dark:text-white truncate text-sm md:text-base">{p.title}</p>
                  ...
                  <div className="flex flex-col sm:flex-row gap-1 sm:gap-3 flex-shrink-0">
                    <button onClick={() => setEditingId(p.id)}
                      className="text-xs md:text-sm font-medium text-blue-500 hover:text-blue-700 cursor-pointer px-2 py-1 rounded border border-blue-200 hover:border-blue-400 transition-colors">
                      Edit
                    </button>
                    <button onClick={() => handleDelete(p.id)}
                      className="text-xs md:text-sm font-medium text-red-500 hover:text-red-700 cursor-pointer px-2 py-1 rounded border border-red-200 hover:border-red-400 transition-colors">
                      Delete
                    </button>
                  </div>
                </div>
